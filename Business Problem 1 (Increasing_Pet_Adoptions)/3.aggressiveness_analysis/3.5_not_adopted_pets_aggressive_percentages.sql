/* 
 Query: Calculate distribution of aggressiveness levels for not adopted pets by species
 Purpose: This query provides counts and percentages for each aggressiveness level (Low, Medium, High) 
 among not adopted pets, segmented by species.
 */
WITH species_totals_not_adopted AS (
    -- Calculate the total count of not adopted pets for each species
    SELECT
        s.species_name,
...
FROM
    pets p
    LEFT JOIN species s ON p.species_id = s.species_id
    LEFT JOIN species_totals_not_adopted t ON s.species_name = t.species_name
WHERE
    p.adopted_status = 'Not Adopted'
GROUP BY
    s.species_name,
